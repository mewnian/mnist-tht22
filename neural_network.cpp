...
int main(int argc, char** argv)
{
    ...
    for (int id = l; id <= r; ++id) 
    {
        ...
        {
            successes++;
            cout << "\033[32m" << "Image #" << id << " | Pred = " << output << ", Answer = " << answer << " | " << "SUCCESS" << endl;
        }
        else
        {
            wrong_ans.emplace_back(id);
            cout << "\033[31m" << "Image #" << id << " | Pred = " << output << ", Answer = " << answer << " | " << "FAILURE" << endl;
        }
        f_data.close(); f_label.close();
    }
    cout << "\033[0m" << "Accuracy = " << (ld)successes / (ld)(r - l + 1) * 100.0l << "%" << " | " << successes << " / " << r - l + 1 << endl;
    cout << "Wrong Preds = "; for (int& v : wrong_ans) cout << v << " ";
}
